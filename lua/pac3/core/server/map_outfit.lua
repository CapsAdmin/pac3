pac.SpawnedMapEntities = pac.SpawnedMapEntities or {}
...
local spawn_handlers = {
	model = function(part, parent)
		...
		then
			SafeRemoveEntity(pac.SpawnedMapEntities[part.self.UniqueID])
			...
			pac.SpawnedMapEntities[part.self.UniqueID] = ent
...
function pac.SpawnMapOutfit(data)
	if data.self then
		data = {data}
	end
...
concommand.Add("pac_spawn_map", function(ply, _, args)
	...
	for k,v in pairs(pac.SpawnedMapEntities) do
		SafeRemoveEntity(v)
	end

	pac.SpawnedMapEntities = {}
