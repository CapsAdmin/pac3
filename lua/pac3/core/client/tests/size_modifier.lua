...
function test.Run(done)
	...
	repeat
		yield()	until GetConVar("pac_modifier_size"):GetBool()

	pacx.SetEntitySizeMultiplier(owner, 0.5)

	repeat
		yield()	until owner:GetModelScale() == 0.5
	...
	repeat
		yield()	until owner:GetModelScale() == 1

	check_shared(owner, "GetCurrentViewOffset", Vector(0, 0, 64))
	RunConsoleCommand("pac_modifier_size", "0")

	repeat
		yield()	until not GetConVar("pac_modifier_size"):GetBool()
