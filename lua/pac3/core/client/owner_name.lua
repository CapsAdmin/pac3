...
function pac.HandleOwnerName(owner, name, ent, part, check_func)
	...
	if name == "world" or name == "worldspawn" then return pac.GetWorldEntity() end
	if name == "self" then return owner end

	if owner:IsValid() then
		if
			name == "active weapon" and
			owner.GetActiveWeapon and
			owner:GetActiveWeapon():IsValid()
		then
			return owner:GetActiveWeapon()
		end

		if name == "active vehicle" and owner.GetVehicle and owner:GetVehicle():IsValid() then return owner:GetVehicle() end
		if name == "hands" and owner == pac.LocalPlayer and pac.LocalHands:IsValid() then return pac.LocalHands end
		if name == "hands" and owner.GetHands then return owner:GetHands() end
		if name == "viewmodel" and owner.GetViewModel then return owner:GetViewModel() end
	end
	...
	if name:find("persist ", nil, true) then
		...
		for _, val in pairs(ents.GetAll()) do
			if
				val.GetPersistent and
				val:GetModel() and
				val:GetPersistent() and
				crc == pac.CalcEntityCRC(val)
			then
				return val
			end
		end
	...
	if
		IsValid(ent) and
		(not check_func or check_func(ent)) and
		check_owner(ent, owner) and
		find_ent(ent, name)
	then
		return ent
	end
	...
	for _, val in pairs(ents.GetAll()) do
		if
			val:IsValid() and
			(not check_func or check_func(val)) and
			check_owner(val, owner) and
			find_ent(val, name)
		then
			return val
		end
