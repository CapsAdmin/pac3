...
function PART:GetNiceName()
	...
	for i, path in ipairs(path:Split(";")) do
		if path ~= "" then
			...
			tbl[i] = pac.PrettifyName(("/" .. path):match(".+/(.-)%.") or path:match("(.-)%.")) or
				"sound"
		end
	end
...
BIND("EchoFeedback", nil, function(n)
	return math.Clamp(n, 0, 0.99)
end)
...
function PART:SetPath(path)
	...
	for _, path in ipairs(paths) do
		...
		if not pac.resource.Download(path, function(path)
			load("data/" .. path)
		end) then
			load("sound/" .. path)
		end
