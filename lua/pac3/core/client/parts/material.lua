	...
	pac.GetSet(PART, "LoadVmt", "", {editor_panel = "material"})
			...
			local function update_submaterial(self, remove, parent)
				pac.RunNextFrameSimple(function()
					...
					if parent:IsValid() then
						...
						elseif
							str ~= "all" and
							parent.GetEntity and
							parent:GetEntity():IsValid() and
							parent:GetEntity():GetMaterials()
						then
			...
			function PART:GetNiceName()
				...
				path = path:gsub("%%(..)", function(char)
					...
					if num then return string.char(num) end
				end)
				...
				for _, v in ipairs(sorted_params) do
					...
					elseif info.type == "texture" then
						...
						PART["Set" .. property_name] = function(self, val)
							...
							if
								val == "" or
								info.partial_hdr and
								mat_hdr_level:GetInt() > 0 and
								self[getnohdr](self)
							then
						...
						pac.GetSet(
							...
							info.default,
							{
								...
								enums = info.enums,
								description = description,
								editor_sensitivity = (info.type == "vec3" or info.type == "color") and
								0.25 or
								nil,
								editor_panel = (info.type == "color" and "color2") or
