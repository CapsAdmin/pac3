		...
		local function callback(channel, errorCode, errorString)
			...
			if not channel or not channel:IsValid() then
				pac.Message(
					"Failed to load ",
					url,
					" (" .. flags .. ") - " .. (errorString or errorCode or "UNKNOWN"))

				if errorCode == -1 then
					pac.Message(
						"GMOD BUG: WAVe and Vorbis files are known to be not working with 3D flag, recode file into MPEG-3 format!")
				end
...
function PART:PlaySound()
	...
	if
		IsValid(self.last_stream) and
		not self.Overlapping and
