...
BUILDER:StartStorableVars()
	BUILDER:SetPropertyGroup("Firing")
		...
		BUILDER:GetSet("Maximum", 0, {description = "PHYSICAL: do not fire if your player has more than this number of active physical projectiles from any projectile part\nNON-PHYSICAL: do not fire if this projectile emitter has more than this number of active projectiles"})
		BUILDER:GetSet("IndependentMaximum", false, {description = "for physical projectiles, decide the maximum based on the part's own projectiles instead of the player's combined total"})
...
function PART:Shoot(pos, ang, multi_projectile_count)
	...
	if physics then
		...
		net.Start("pac_projectile",true)
			...
			net.WriteBool(self.IndependentMaximum)
