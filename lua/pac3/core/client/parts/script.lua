	...
	local obj = setmetatable(
		{},
		{
			__index = function(_, key)
				...
				if store.parts[part.UniqueID][key] then return store.parts[part.UniqueID][key] end
				return META[key]
			end,
...
function PART:CompileCode()
	...
	for _, word in pairs(blacklist) do
		if code:find("[%p%s]" .. word) or code:find(word .. "[%p%s]") then return false, string.format("illegal characters used %q", word) end
	end

	local func = CompileString(code, "SCRIPT_ENV", false)
	if type(func) == "string" then return false, func end
	...
	local extra_lib = {
			...
			owner = {
				GetFOV = function()
					...
					if ent:IsValid() then return ent:GetFOV() end
				end,
				GetHealth = function()
					local ent = get_entity(self)
					if ent:IsValid() then return ent:Health() end
				end,
			},
			parts = {
				...
				FindMultiple = function(str)
					...
					for _, part in pairs(pac.GetParts()) do
						if part:GetPlayerOwner() == self:GetPlayerOwner() and pac.StringFind(part:GetName(), str) then
	...
	env.__index = function(_, key)
		if key == "this" or key == "self" then return CreateDummy(self, store, self) end
		if key == "T" or key == "TIME" then return RealTime() end
		if key == "CT" or key == "CURTIME" then return CurTime() end
		if lib[key] then return lib[key] end
		if extra_lib[key] then return extra_lib[key] end
