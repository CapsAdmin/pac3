...
function haloex.Add(ents, color, blurx, blury, passes, add, ignorez, amount, spherical, shape)
	...
	local t = {
			...
			BlurY = blury or
			2,
			DrawPasses = passes or
			1,
			Additive = add,
			IgnoreZ = ignorez,
			Amount = amount or
			1,
			SphericalSize = spherical or
			1,
			Shape = shape or
			1,
		}
...
function haloex.Render(entry)
 ...
 -- don't render any colour
	...
	for i = 0, entry.DrawPasses do
		...
		render.DrawScreenQuadEx(math.Clamp(x, s * -entry.Shape, s * entry.Shape), math.Clamp(y, s * -entry.Shape, s * entry.Shape), ScrW(), ScrH())
	end
