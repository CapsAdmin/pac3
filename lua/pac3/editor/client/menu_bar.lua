...
local function populate_pac(menu)
	...
	local clear, pnl = menu:AddSubMenu(L"clear", function() 
	end)
	pnl:SetImage(pace.MiscIcons.clear)
	clear.GetDeleteSelf = function()
		return false
	end
	clear:AddOption(L"OK", function()
		pace.ClearParts()
	end):SetImage(pace.MiscIcons.clear)
	menu:AddSpacer()
	local help, help_pnl = menu:AddSubMenu(L"help", function()
		pace.ShowWiki()
	end)
	help.GetDeleteSelf = function()
		return false
	end
	help_pnl:SetImage(pace.MiscIcons.help)
	help:AddOption(L"Getting Started", function()
		pace.ShowWiki(pace.WikiURL .. "Beginners-FAQ")
	end)
	:SetImage(pace.MiscIcons.info)
	local chat_pnl = help:AddOption(L"Discord / PAC3 Chat", function()
		gui.OpenURL("https://discord.gg/utpR3gJ")
	...
	if info then
		local version, version_pnl = help:AddSubMenu(L"Version", function()
			pace.ShowWiki()
		end)
		version.GetDeleteSelf = function()
			return false
		end
	...
	if cookie.GetNumber("pac3_discord_ad", 0) < 3 then
		help_pnl.PaintOver = function(_, w, h)
			surface.SetDrawColor(255, 255, 0, 50 + math.sin(SysTime() * 20) * 20)
	...
	menu:AddOption(L"exit", function()
		pace.CloseEditor()
	end):SetImage(pace.MiscIcons.exit)
end

local function populate_view(menu)
	menu:AddOption(L"hide editor", function()
		pace.Call("ToggleFocus")
		chat.AddText("[PAC3] \"ctrl + e\" to get the editor back")
	end)
	:SetImage("icon16/application_delete.png")
	menu:AddCVar(L"camera follow", "pac_camera_follow_entity", "1", "0"):SetImage("icon16/camera_go.png")
	menu:AddOption(L"reset view position", function()
		pace.ResetView()
	end):SetImage("icon16/camera_link.png")
	menu:AddOption(L"reset zoom", function()
		pace.ResetZoom()
	end):SetImage("icon16/magnifier.png")
	menu:AddOption(L"about", function()
		pace.ShowAbout()
	end):SetImage(pace.MiscIcons.about)
end

local function populate_options(menu)
	menu:AddCVar(L"advanced mode", "pac_basic_mode", "0", "1").DoClick = function()
	...
	menu:AddCVar(L"render attachments as bones", "pac_render_attachments", "1", "0").DoClick = function()
		pace.ToggleRenderAttachments()
	end
	menu:AddSpacer()
	...
	menu:AddSpacer()
	local rendering, pnl = menu:AddSubMenu(L"rendering", function() 
	end)
	rendering.GetDeleteSelf = function()
		return false
	end
...
local function populate_player(menu)
	local pnl = menu:AddOption(L"t pose", function()
	...
	if pacx and pacx.GetServerModifiers then
		local mods, pnl = menu:AddSubMenu(L"modifiers", function() 
		end)
		pnl:SetImage("icon16/user_edit.png")
		mods.GetDeleteSelf = function()
			return false
		end
