...
do
	...
	PART.PropertyWhitelist = {"Position", "Angles", "Bone",}
...
function timeline.EditBone()
	...
	timeline.selected_bone = timeline.dummy_bone and
		...
		boneData[timeline.dummy_bone:GetBone()].real or
		false
	...
	timer.Create("pace_timeline_save", 0.1, 1, function()
		if part and part:IsValid() then
			...
			if part:GetURL() ~= "" then
				...
				timer.Create("pace_backup", 1, 1, function()
					pace.Backup()
				end)
			end
		end
				...
				save.DoClick = function()
					Derma_StringRequest(
						...
						function(name)
							...
							file.Write("pac3/__animations/" .. name .. ".txt", util.TableToJSON(timeline.data))
						end,
						function() 
						end,
						L"save",
	...
	function KEYFRAME:OnMousePressed(mc)
		...
		elseif mc == MOUSE_RIGHT then
			...
			if not self:GetRestart() then
				...
				:SetImage("icon16/control_repeat.png")
			end
