...
do -- forcing hooks
	pace.ExternalHooks = {"CalcView", "ShouldDrawLocalPlayer",}
...
concommand.Add("pac_reset_eye_angles", function()
	pace.ResetEyeAngles()
end)

concommand.Add("pac_toggle_tpose", function()
	pace.SetTPose(not pace.GetTPose())
end)

function pace.Call(str, ...)
	if pace["On" .. str] then
		if hook.Run("pace_On" .. str, ...) ~= false then return pace["On" .. str](...) end
	else
...
do
	...
	do
		...
		timer.Create("pac_in_editor", 0.25, 0, function()
			...
			local target_pos = (pace.mctrl.GetTargetPos()) or
				pace.current_part:GetDrawPosition() or
				vector_origin
