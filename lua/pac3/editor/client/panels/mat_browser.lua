...
function PANEL:SetMaterialList(tbl)
	...
	for i, material in pairs(self.MaterialList) do
		...
		image.m_Image.LoadMaterial = function(s)
			s:DoLoadMaterial()
		end
		...
		image.DoClick = function(image)
			self:SetSelected(image.Value)
		end
...
function PANEL:Init()
	...
	local list = {
			{
				key = "default",
				val = table.Merge(list.Get("OverrideMaterials"), pace.Materials.materials),
			},
	...
	for _, data in pairs(list) do
		...
		pnl.SetSelected = function(_, path)
	...
	entry.OnEnter = function()
		self:SetSelected(self.Entry:GetValue())
	end
	entry:Dock(BOTTOM)
	entry:SetTall(20)
	self.Entry = entry
...
function PANEL:MaterialSelected(path) 
end
