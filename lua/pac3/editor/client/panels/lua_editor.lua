...
function PANEL:Init()
	...
	function PANEL:SetCaretPos(x, y)
		...
		function PANEL:OnMousePressed(code)
			if (code == MOUSE_LEFT) then
				if
					(
						(CurTime() - self.LastClick) < 1 and
						self.tmp and
						self:CursorToCaret()[1] == self.Caret[1] and
						self:CursorToCaret()[2] == self.Caret[2]
					)
				then
	...
	colors = {
						...
						while self.char do
							...
							if (self.char >= "0" and self.char <= "9") then
								...
								token = "number"
							elseif
								(self.char >= "a" and self.char <= "z" or self.char >= "A" and self.char <= "Z")
							then
								while self.char and
								(
									self.char >= "a" and self.char <= "z" or self.char >= "A" and self.char <= "Z" or
									self.char >= "0" and self.char <= "9" or
									self.char == "_"
								) do
									self:NextChar()
								end
								...
								elseif
									(
										self:CheckGlobal(sstr) and
										(
											type(self:CheckGlobal(sstr)) == "function" or
											self:CheckGlobal(sstr) == "f" or
											self:CheckGlobal(sstr) == "e" or
											self:CheckGlobal(sstr) == "m" or
											type(self:CheckGlobal(sstr)) == "table"
										) or
										(lasttable and lasttable[sstr])
									)
								then -- Could be better code, but what the hell; it works
	...
	local rows = string.Explode("\n", text)
											...
											local stop = {
													start[1] + #rows - 1,
													string.len(self.Rows[start[1] + #rows - 1]) + 1,
												}
																												...
																												elseif (code == KEY_END) then
																													...
																													elseif (code == KEY_BACKSPACE) then
																														if (self:HasSelection()) then
																															...
																															if
																																(self.Caret[2] % 4 == 1 and string.len(buffer) > 0 and string.rep(" ", string.len(buffer)) == buffer)
																															then
																													...
																													elseif (code == KEY_DELETE) then
																														if (self:HasSelection()) then
																															...
																															if
																																(
																																	self.Caret[2] % 4 == 1 and
																																	string.rep(" ", string.len(buffer)) == buffer and
																																	string.len(self.Rows[self.Caret[1]]) >= self.Caret[2] + 4 - 1
																																)
																															then
																																self:SetCaret(self:SetArea({self.Caret, self:MovePosition(self.Caret, 4)}))
																															else
																												...
																												if (code == KEY_TAB or (control and (code == KEY_I or code == KEY_O))) then
																													...
																													function PANEL:getWordStart(caret)
																														...
																														for i = 0, caret[2] do
																															...
																															if
																																(
																																	line[caret[2] - i] >= "a" and
																																	line[caret[2] - i] <= "z" or
																																	line[caret[2] - i] >= "A" and
																																	line[caret[2] - i] <= "Z" or
																																	line[caret[2] - i] >= "0" and
																																	line[caret[2] - i] <= "9"
																																)
																															then
