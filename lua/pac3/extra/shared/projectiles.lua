		...
		function ENT:PhysicsUpdate(phys)
			...
			if self.part_data.Attract ~= 0 then
				...
				elseif self.part_data.AttractMode == "hitpos_radius" then
					...
					phys:SetVelocity(phys:GetVelocity() + dir)
				elseif
					self.part_data.AttractMode == "closest_to_projectile" or
					self.part_data.AttractMode == "closest_to_hitpos"
				then
					if self.next_target < CurTime() then
						...
						for _, ent in ipairs(ents.FindInSphere(pos, radius)) do
							...
							then
								local data = {
										dist = ent:NearestPoint(ent:GetPos()):Distance(pos),
										ent = ent,
									}
						...
						if closest_1[1] then
							table.sort(closest_1, function(a, b)
								return a.dist < b.dist
							end)

							self.target_ent = closest_1[1].ent
						elseif closest_2[1] then
							table.sort(closest_2, function(a, b)
								return a.dist < b.dist
							end)
