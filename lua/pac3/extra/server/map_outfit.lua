...
local spawn_handlers = {
		model = function(part, parent)
			...
			then
				...
				if data.Color then
					ent:SetColor(tocolor(c, (data.Alpha or 1) * 255))
				end
		...
		light = function(part, parent)
			...
			if data.Size then
				ent:SetKeyValue("distance", data.Size)
			end

			if data.Style then
				ent:SetKeyValue("style", data.Style)
			end
